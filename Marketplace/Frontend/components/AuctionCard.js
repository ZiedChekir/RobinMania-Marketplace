...
export default function AuctionCard({
  ...
  const startTimer = () => {
    interval = setInterval(() => {
      ...
      if (distance >= 0) {
        ...
        setProgress(
          (100 * (now - auction.auctionStartedAt)) /
            (auction.auctionEnd - auction.auctionStartedAt)
        );
      } else {
  ...
  const placeBid = async () => {
    ...
    const result = await contract.placeBid(auction.tokenID, auction.index, {
  ...
  const Claim = async () => {
    ...
    if (result["hash"].length == 66)
      toast.success("Successfully claimed the item!");
    else toast.error("error");
    setPlaceBidStatus(false);
  };
  ...
  const truncateAddress = (address) => {
    return (
      address.substring(0, 4) + "..." + address.substring(39, address.length)
    );
  };
  ...
  return (
    <Card
      ...
      sx={{ maxWidth: 320, color: "#1EB854", backgroundColor: "#272935" }}
      elevation={10}
    >
      <CardActionArea>
        <CardMedia component="img" image={tokenImg} />
        <CardContent>
      ...
      <CardActions>
        ...
        <Stack style={{ paddingLeft: 100 }}>
          <Typography>NFT Owner :</Typography>
          <Typography>{truncateAddress(auction.seller)}</Typography>
        </Stack>
      </CardActions>
      <Dialog open={placeBidStatus} onClose={() => setPlaceBidStatus(false)}>
        ...
        {CardType == 1 && (
          <DialogContent sx={{ width: 250 }}>
            <DialogContentText>Claim your item</DialogContentText>
          </DialogContent>
        )}
        <DialogActions>
