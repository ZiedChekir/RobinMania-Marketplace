async function main() {
  ...
  for (let i = 1; i < 6; i++) {
    await auction.createNewAuctionItem(nft.address, i, "100000000", 60, "1000");
  }
  ...
  await auction
    .connect(account1)
    .placeBid(1, 0, { value: "9990000000000000000" });
  console.log("user 2 place a bid on Auction ");

  await auction
    .connect(account2)
    .placeBid(1, 0, { value: "99999998797299492708" });
  ...
  // }, 60000);
  console.log("----------------printing all auctions---------------");
  console.log(await auction.getAllAuctions(5));
}
